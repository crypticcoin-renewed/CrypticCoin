...
TEST(Keys, EncodeAndDecodeSapling)
{
    ...
    for (uint32_t i = 0; i < 1000; i++) {
        auto sk = m.Derive(i);
        {
            ...
            auto sk2 = std::get_if<libcrypticcoin::SaplingExtendedSpendingKey>(&spendingkey2.value());
        ...
        }
        {
            ...
            auto extfvk2 = std::get_if<libcrypticcoin::SaplingExtendedFullViewingKey>(&viewingkey2.value());
        ...
        }
        {
            ...
            auto addr2 = std::get_if<libcrypticcoin::SaplingPaymentAddress>(&paymentaddr2.value());
...
TEST(Keys, EncodeAndDecodeUnifiedAddresses)
{
    ...
    for (size_t idx = 0; idx < ua_tests.size(); idx++) {
        ...
        try {
            libcrypticcoin::UnifiedAddress ua;
            ...
            if (!test[3].isNull()) {
                ...
                libcrypticcoin::UnknownReceiver r(0x03, data);
                ua.AddReceiver(r);
            }
            if (!test[2].isNull()) {
                ...
                CDataStream ss(
                    ...
                    SER_NETWORK,
                    PROTOCOL_VERSION);
                libcrypticcoin::SaplingPaymentAddress r;
                ss >> r;
                ua.AddReceiver(r);
            }
            if (!test[1].isNull()) {
            ...
            auto ua_ptr = std::get_if<libcrypticcoin::UnifiedAddress>(&decoded.value());
...
TEST(Keys, DeriveUnifiedFullViewingKeys)
{
    ...
    for (size_t idx = 0; idx < ua_tests.size(); idx++) {
        ...
        try {
            ...
            auto usk = libcrypticcoin::CrypticcoindUnifiedSpendingKey::ForAccount(
                    hdseed,
                    133, //CRYPTICCOIN_MAIN_COINTYPE
                    test[7].get_int());
            ...
            if (!test[0].isNull()) {
                ...
                auto expected = libcrypticcoin::transparent::AccountPubKey(CChainablePubKey::Read(ss));
            ...
            if (!test[1].isNull()) {
                ...
                CDataStream ss(data, SER_NETWORK, PROTOCOL_VERSION);
                auto key = libcrypticcoin::SaplingDiversifiableFullViewingKey::Read(ss);
...
TEST(Keys, EncodeAndDecodeUnifiedFullViewingKeys)
{
    ...
    for (size_t idx = 0; idx < ua_tests.size(); idx++) {
        ...
        libcrypticcoin::UnifiedFullViewingKeyBuilder builder;
        ...
        auto decoded = libcrypticcoin::UnifiedFullViewingKey::Decode(keystr, Params());
